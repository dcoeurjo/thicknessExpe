...
class powercrust {  // class that contains the whole powercrust.
  ...
  void dealEdge(int,int,int,std::stack<int>&);
...
void powercrust::addFace(std::vector<int>& indices){
...
void powercrust::readInput(std::ifstream& inFile) {
...
void powercrust::correctOrientation() { 
  ...
  std::stack<int> remFaces;
  std::stack<int> allFaces;
  ...
  for(int i=0;i<faces.size();i++) {
    ...
    if(faces[i].visited==false) {
      ...
      std::cout<<"starting a new component ..\n";
      remFaces.push(i);
      ...
      std::cout<<"The number of faces with the two orientations are "<<Or[0]<<"\t"<<Or[1]<<"\n";
      if(Or[0]>Or[1])
        corr=0;
      else
        corr=1;
...
void powercrust::dealEdge(int index1,int index2,int faceindex,std::stack<int>& remFaces) {
...
void powercrust::writeOutput(std::ofstream& outFile) {
