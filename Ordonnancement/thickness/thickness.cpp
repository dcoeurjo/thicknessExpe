...
// A class for the "CGAL::boc_intersection_d" call
...
int main( int argc, char * argv[] ) {
    /* 
    argv[1] = .OFF file containing the mesh
    ...
    */
    ...
    // Build spheres
    std::string systemCall = "bash off2spheres.sh "+filename+" "+intToString(resolution) ;
    int s = system(systemCall.c_str());
    if ( s != 0 ) {
        std::cerr << "Error when call off2spheres" << std::endl ;
        return -1 ;
    }
    // Spheres are create with filename  ${OFFNAME%%.off}-$2-spheres.txt ( cf off2spheres )
    std::string sphereName = filename.substr(0,filename.size()-4)+"-"+intToString(resolution)+"-spheres.txt" ;
    Spheres spheres = readSpheres(sphereName);
